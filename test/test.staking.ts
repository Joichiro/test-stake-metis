...
describe('Staking', () => {
  ...
  const STAKE_AMOUNT: number = 1000;
  ...
  beforeEach(async () => {
    ...
    Token = await ethers.getContractFactory('TestMetisToken');
    token = await Token.deploy('TestMetisToken', 'MTT', ethers.utils.parseEther(MINT_AMOUNT.toString()));
  ...
  it('should allow users to stake tokens', async () => {
    ...
    expect(await staking.connect(addr1).deposit(ethers.utils.parseEther(STAKE_AMOUNT.toString()))).to.not.reverted;
  });
  ...
  it('should allow users to unstake tokens', async () => {
    ...
    expect(await staking.connect(addr1).withdraw(ethers.utils.parseEther('0'))).not.be.reverted;
  });

  it('should calculate rewards correctly', async () => {
    await token.connect(addr1).approve(staking.address, ethers.utils.parseEther(STAKE_AMOUNT.toString()));
    await staking.connect(addr1).deposit(ethers.utils.parseEther(STAKE_AMOUNT.toString()));
